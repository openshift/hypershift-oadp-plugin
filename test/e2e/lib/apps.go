...
func AreVolumeSnapshotsReady(ocClient client.Client, backupName string) wait.ConditionFunc {
	return func() (bool, error) {
		...
		for _, v := range vList.Items {
			log.Printf("waiting for volume snapshot contents %s to be ready\n", v.Name)
...
func AreApplicationPodsRunning(c *kubernetes.Clientset, namespace string) wait.ConditionFunc {
	return func() (bool, error) {
		podList, err := helpers.GetAllPodsWithLabel(c, namespace, e2eAppLabel)
		if err != nil {
			return false, err
		}
...
func VerifyBackupRestoreData(ocClient client.Client, kubeClient *kubernetes.Clientset, kubeConfig *rest.Config, artifactDir string, namespace string, routeName string, serviceName string, app string, prebackupState bool, twoVol bool) error {
	...
	requestParamsTodoIncomplete := getRequestParameters(appEndpointURL+"/todo-incomplete", proxyPodParams, helpers.GET, nil)

	if prebackupState {
		// Clean up existing backup file
		helpers.RemoveFileIfExists(artifactDir + "/backup-data.txt")

		// Make requests and update data before backup
		dataBeforeCurl, errResp, err := helpers.MakeRequest(*requestParamsTodoIncomplete)
		if err != nil {
			if errResp != "" {
		...
		requestParams := getRequestParameters(appEndpointURL+"/todo", proxyPodParams, helpers.POST, &postPayload)
		helpers.MakeRequest(*requestParams)
	...
	respData, errResp, err := helpers.MakeRequest(*requestParamsTodoIncomplete)
	if err != nil {
		if errResp != "" {
			log.Printf("Request response error msg: %s\n", errResp)
		}
	...
	if twoVol {
		...
		requestParamsVolume := getRequestParameters(appEndpointURL+"/log", proxyPodParams, helpers.GET, nil)
...
func getRequestParameters(url string, proxyPodParams *helpers.ProxyPodParameters, method helpers.HTTPMethod, payload *string) *helpers.RequestParameters {
	return &helpers.RequestParameters{
...
func getAppEndpointURLAndProxyParams(ocClient client.Client, kubeClient *kubernetes.Clientset, kubeConfig *rest.Config, namespace, serviceName, routeName string) (string, *helpers.ProxyPodParameters, error) {
	appEndpointURL, err := helpers.GetRouteEndpointURL(ocClient, namespace, routeName)
	...
	if err != nil {
		...
		pod, podErr := helpers.GetFirstPodByLabel(kubeClient, namespace, "curl-tool=true")
		if podErr != nil {
			return "", nil, fmt.Errorf("Error getting pod for the proxy command: %v", podErr)
		}

		proxyPodParams := &helpers.ProxyPodParameters{
		...
		appEndpointURL = helpers.GetInternalServiceEndpointURL(namespace, serviceName)
...
func verifyVolume(requestParams *helpers.RequestParameters, volumeFile string, prebackupState bool) error {
	var volData string
	var err error
	...
	getResponseFromVolumeCall := func() bool {
		...
		var errResp string
		volData, errResp, err = helpers.MakeRequest(*requestParams)
		if err != nil {
			if errResp != "" {
				log.Printf("Request response error msg: %s\n", errResp)
			}
	...
	if prebackupState {
		...
		helpers.RemoveFileIfExists(volumeFile)
