...
func waitOADPReadiness(backupRestoreType helpers.BackupRestoreType) {
	...
	gomega.Eventually(helpers.VeleroPodIsRunning(kubernetesClientForSuiteRun, namespace), time.Minute*3, time.Second*5).Should(gomega.BeTrue())

	if backupRestoreType == helpers.RESTIC || backupRestoreType == helpers.KOPIA || backupRestoreType == helpers.CSIDataMover {
		log.Printf("Waiting for Node Agent pods to be running")
		gomega.Eventually(helpers.AreNodeAgentPodsRunning(kubernetesClientForSuiteRun, namespace), time.Minute*3, time.Second*5).Should(gomega.BeTrue())
	}
...
func prepareBackupAndRestore(brCase BackupRestoreCase, updateLastInstallTime func()) (string, string) {
	...
	if brCase.BackupRestoreType == helpers.CSI || brCase.BackupRestoreType == helpers.CSIDataMover {
...
func runApplicationBackupAndRestore(brCase ApplicationBackupRestoreCase, updateLastBRcase func(brCase ApplicationBackupRestoreCase), updateLastInstallTime func()) {
	...
	gomega.Eventually(helpers.IsNamespaceDeleted(kubernetesClientForSuiteRun, brCase.Namespace), time.Minute*4, time.Second*5).Should(gomega.BeTrue())
...
func runBackup(brCase BackupRestoreCase, backupName string) bool {
	...
	err = lib.CreateBackupForNamespaces(dpaCR.Client, namespace, backupName, []string{brCase.Namespace}, brCase.BackupRestoreType == helpers.RESTIC || brCase.BackupRestoreType == helpers.KOPIA, brCase.BackupRestoreType == helpers.CSIDataMover)
	gomega.Expect(err).ToNot(gomega.HaveOccurred())
...
func getFailedTestLogs(oadpNamespace string, appNamespace string, installTime time.Time, report ginkgo.SpecReport) {
	...
	err = helpers.SavePodLogs(kubernetesClientForSuiteRun, oadpNamespace, baseReportDir)
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	...
	if appNamespace != "" {
		...
		err = helpers.SavePodLogs(kubernetesClientForSuiteRun, appNamespace, baseReportDir)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
	}
}
...
func tearDownBackupAndRestore(brCase BackupRestoreCase, installTime time.Time, report ginkgo.SpecReport) {
	...
	if report.Failed() {
		knownFlake = helpers.CheckIfFlakeOccurred(accumulatedTestLogs)
	...
	err = helpers.DeleteNamespace(kubernetesClientForSuiteRun, brCase.Namespace)
	gomega.Expect(err).ToNot(gomega.HaveOccurred())
	gomega.Eventually(helpers.IsNamespaceDeleted(kubernetesClientForSuiteRun, brCase.Namespace), time.Minute*5, time.Second*5).Should(gomega.BeTrue())
}
...
var _ = ginkgo.Describe("Backup and restore tests", ginkgo.Ordered, func() {
	...
	var _ = ginkgo.AfterAll(func() {
		...
		waitOADPReadiness(helpers.KOPIA)
	...
	ginkgo.DescribeTable("Backup and restore applications",
		...
		ginkgo.Entry("Mongo application RESTIC", ginkgo.FlakeAttempts(flakeAttempts), ApplicationBackupRestoreCase{
			...
			BackupRestoreCase: BackupRestoreCase{
				...
				BackupRestoreType: helpers.RESTIC,
				PreBackupVerify:   todoListReady(true, false, "mongo"),
				PostRestoreVerify: todoListReady(false, false, "mongo"),
				BackupTimeout:     20 * time.Minute,
			},
		...
		ginkgo.Entry("Mongo application KOPIA", ginkgo.FlakeAttempts(flakeAttempts), ApplicationBackupRestoreCase{
			...
			BackupRestoreCase: BackupRestoreCase{
				...
				BackupRestoreType: helpers.KOPIA,
				PreBackupVerify:   todoListReady(true, false, "mongo"),
				PostRestoreVerify: todoListReady(false, false, "mongo"),
				BackupTimeout:     20 * time.Minute,
			},
		...
		ginkgo.Entry("MySQL application Native-Snapshots", ginkgo.FlakeAttempts(flakeAttempts), ginkgo.Label("aws", "azure", "gcp"), ApplicationBackupRestoreCase{
			...
			BackupRestoreCase: BackupRestoreCase{
				...
				BackupRestoreType: helpers.NativeSnapshots,
				PreBackupVerify:   todoListReady(true, false, "mysql"),
				PostRestoreVerify: todoListReady(false, false, "mysql"),
				BackupTimeout:     20 * time.Minute,
			},
